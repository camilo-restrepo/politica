	...
	$scope.init = function(){
		...
		d3.json("resources/unidades.json", function(error, bogota) {
			...
			.on("mouseover", function(d) { 
				...
				div.transition().duration(200).style("opacity", .9);
				div.style("z-index", "2000").style("width", "150px").style("left", (d.centroid[0] - 200) + "px")
				.style("top", (d.centroid[1] + 20) + "px").style("height", d3.select("div.tooltip").node().getBoundingClientRect().height);
            })          
			.on("mouseout", function() {   
				div.transition().duration(500).style("opacity", 0); 
			});			
